...
namespace tgk_planner
{
  ...
  public:
    ...
    bool checkRayValid(const Eigen::Vector3d &p0, const Eigen::Vector3d &p1, double max_dist = DBL_MAX) const
    {
      ...
      while (raycaster.step(ray_pt))
      {
        ...
        if (occ_map_->isInflateOccupied(tmp))
        {
          return false;
        }
      }
  ...
  inline bool PosChecker::validatePosSurround(const Vector3d &pos)
  {
    ...
    if (occ_map_->isInflateOccupied(pos))
      return false;
    return true;
  }
  ...
  inline bool PosChecker::validatePosSurround(const Vector3i &id)
  {
    ...
    if (occ_map_->isInflateOccupied(id))
      return false;
    return true;
  }
