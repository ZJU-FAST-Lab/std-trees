...
namespace tgk_planner
{
  ...
  void FSM::init(const ros::NodeHandle &nh)
  {
    ...
    // krrt_sharp_ptr_.reset(new BIKRRT(nh));
  ...
  void FSM::executionCallback(const ros::TimerEvent &event)
  {
    ...
    case GENERATE_TRAJ:
    {
      bool success = searchForTraj(start_pos_, start_vel_, start_acc_, end_pos_, end_vel_, end_acc_, replan_time_); //TODO what if it can not finish in 10ms?
      if (success)
      {
    ...
    case FOLLOW_TRAJ:
    {
      ...
      StatePVA curr_expected_state(9);
                          ...
                          double search_time)
  {
    ...
    string file_name("/home/kai/Pictures/a.csv");
    std::ofstream o_file;
    ...
    // result = krrt_sharp_ptr_->plan(start_pos, start_vel, start_acc, end_pos, end_vel, end_acc, search_time, NO_DEFORM, 0, 0);
    if (result == KRRTSTAR::SUCCESS)
    {
      double traj_use_time(0.0);
      krrt_sharp_ptr_->getTraj(traj_);
      traj_use_time = krrt_sharp_ptr_->getFinalTrajTimeUsage();
      double traj_cost = krrt_sharp_ptr_->evaluateTraj(traj_, traj_duration, traj_len, traj_seg_nums, traj_acc_itg, traj_jerk_itg);
      ROS_INFO_STREAM("[      ]: [front-end final path]: " << endl
                                                           << "    -   seg nums: " << traj_seg_nums << endl
                                                           << "    -   acc integral: " << traj_acc_itg << endl
                                                           << "    -   jerk integral: " << traj_jerk_itg << endl
                                                           << "    -   traj duration: " << traj_duration << endl
                                                           << "    -   traj cost: " << traj_cost << endl
                                                           << "    -   path length: " << traj_len << " m");
      ROS_INFO_STREAM("[      ]: [front-end final path]: " << endl
                                                           << "    -   max vel: " << traj_.getMaxVelRate() << endl
                                                           << "    -   max acc: " << traj_.getMaxAccRate() << endl
                                                           << "    -   max jrk: " << traj_.getMaxJerkRate() << " m");
      vis_x.clear();
      ...
      o_file.open(file_name, ios::out | ios::app);
      for (size_t i = 0; i < traj_list.size(); ++i)
      {
        ...
        o_file << solution_cost_list[i] << ","
               << solution_time_list[i] << ","
               << endl;
      }
      o_file << endl;
      o_file.close();
    ...
    krrt_sharp_ptr_->reset();
    int result2(false);
    result2 = krrt_sharp_ptr_->plan(start_pos, start_vel, start_acc, end_pos, end_vel, end_acc, search_time, SMALL_BRANCH_NODES, 0, 1);
    if (result2 == KRRTSTAR::SUCCESS)
    {
      double traj_use_time(0.0);
      krrt_sharp_ptr_->getTraj(traj_);
      traj_use_time = krrt_sharp_ptr_->getFinalTrajTimeUsage();
      double traj_cost = bikrrt_ptr_->evaluateTraj(traj_, traj_duration, traj_len, traj_seg_nums, traj_acc_itg, traj_jerk_itg);
      ROS_INFO_STREAM("[DEFORM]: [front-end final path]: " << endl
                                                           << "    -   seg nums: " << traj_seg_nums << endl
                                                           << "    -   acc integral: " << traj_acc_itg << endl
                                                           << "    -   jerk integral: " << traj_jerk_itg << endl
                                                           << "    -   traj duration: " << traj_duration << endl
