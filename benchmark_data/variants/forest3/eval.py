...
ax.scatter(eval_timeline, cost_mean_var[0], marker='o', s=100, label='NODE')
ax.errorbar(eval_timeline, cost_mean_var[0], yerr=cost_mean_var[1], capsize=2, lw=1.5)

book = xlrd.open_workbook("./f-trunk.xls")
eval_timeline = np.array([0, 100, 200, 400, 700, 1100, 2100, 3000])
cost_mean_var = get_cost_mean_var_list(book, eval_timeline)
ax.scatter(eval_timeline, cost_mean_var[0], marker='v', s=100, label='TRUNK')
ax.errorbar(eval_timeline, cost_mean_var[0], yerr=cost_mean_var[1], capsize=2, lw=1.5)
...
cost_mean_var = get_cost_mean_var_list(book, eval_timeline)
ax.scatter(eval_timeline, cost_mean_var[0], marker='X', s=100, label='TREE')
ax.errorbar(eval_timeline, cost_mean_var[0], yerr=cost_mean_var[1], capsize=2, lw=1.5)

book = xlrd.open_workbook("./f-small-branch.xls")
eval_timeline = np.array([0, 120, 225, 345, 750, 1150, 2150, 3000])
cost_mean_var = get_cost_mean_var_list(book, eval_timeline)
ax.scatter(eval_timeline, cost_mean_var[0], marker='P', s=100, label='BRANCH')
ax.errorbar(eval_timeline, cost_mean_var[0], yerr=cost_mean_var[1], capsize=2, lw=1.5)

book = xlrd.open_workbook("./f-no-deform.xls")
eval_timeline = np.array([0, 60, 150, 450, 600, 1000, 2000, 3000])
cost_mean_var = get_cost_mean_var_list(book, eval_timeline)
ax.scatter(eval_timeline, cost_mean_var[0], marker='d', s=80,  label='w/o')
ax.errorbar(eval_timeline, cost_mean_var[0], yerr=cost_mean_var[1], capsize=2, lw=1.5)
...
time_all = np.array(sheet.col_values(1))
cost_all = np.array(sheet.col_values(2))
ax.scatter(time_all, cost_all, marker='x', s=70, c='b', label='Search')
