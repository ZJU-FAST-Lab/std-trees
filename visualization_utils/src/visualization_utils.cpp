...
VisualRviz::VisualRviz(const ros::NodeHandle &nh) : nh_(nh)
{
    ...
    curr_exp_pos_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("/curr_exp_pos", 1);
    curr_exp_vel_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("/curr_exp_vel", 1);
    curr_exp_acc_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("/curr_exp_acc", 1);
}
...
void VisualRviz::visualizeStates(const std::vector<StatePVA> &x, int trajectory_type, ros::Time local_time)
{
    ...
    case TreeTraj:
        pos_point.color = Color::Blue();
        pos_point.color.a = 1.0;
        pos_point.scale.x = 0.06;
        pos_point.scale.y = 0.06;
        pos_point.scale.z = 0.06;
        pos_point.type = visualization_msgs::Marker::POINTS;
        vel_vec.color = Color::SteelBlue();
        vel_vec.color.a = 1.0;
        acc_vec.color = Color::SteelBlue();
...
void VisualRviz::visualizeKnots(const std::vector<Eigen::Vector3d> &knots, ros::Time local_time)
{
    ...
    pos_point.scale.x = 0.25;
    pos_point.scale.y = 0.25;
    pos_point.scale.z = 0.25;
    pos_point.color = Color::Orange();
    pos_point.color.a = 1.0;
...
void VisualRviz::visualizeTrajList(const std::vector<std::vector<StatePVA>> &x, ros::Time local_time)
{
    ...
    for (size_t i = 0; i < traj_num; ++i)
    {
        ...
        traj_point.color.g = 1 - (double)(i + 1 + 3) / (double)(traj_num + 10);
        traj_point.color.b = 1 - (double)(i + 1 + 3) / (double)(traj_num + 10);
        traj_point.color.a = (double)(i + 1 + 5) / (double)(traj_num + 5);
...
void VisualRviz::visualizeText(const std::vector<std::string> &texts, const std::vector<Eigen::Vector3d> &positions, ros::Time local_time)
{
    ...
    double y_diff(-20);
    double vertical_diff(-2.5);
    double x_base(-3), z_base(4);
    /*line 1*/
    int line_num = 3;
    text_mk.color = Color::Black();
    text_mk.text = "Planning Time(ms)";
    text_mk.pose.position.x = x_base + vertical_diff * line_num;
    text_mk.pose.position.y = y_diff;
    text_mk.pose.position.z = 3;
    text_mk.id = 0;
    mk_ary.markers.push_back(text_mk);

    text_mk.text = "Traj.Cost";
    text_mk.pose.position.x = x_base + vertical_diff * line_num;
...
void VisualRviz::visualizeCurrExpectedState(const std::vector<StatePVA> &x, ros::Time local_time)
{
    if (x.empty())
        return;
    visualization_msgs::MarkerArray p_list, v_list, a_list;
    visualization_msgs::Marker clear_previous_msg;
    clear_previous_msg.action = visualization_msgs::Marker::DELETEALL;
    p_list.markers.push_back(clear_previous_msg);
    v_list.markers.push_back(clear_previous_msg);
    a_list.markers.push_back(clear_previous_msg);

    size_t state_num = x.size();
    for (size_t i = 0; i < state_num; ++i)
    {
        ...
        pos_point.pose.orientation.w = 1.0;
        pos_point.id = i;
        pos_point.type = visualization_msgs::Marker::MESH_RESOURCE;
        std::string spherePath = "package://visualization_utils/sphere.stl";
        pos_point.mesh_resource = spherePath;
        pos_point.scale.x = 0.5;
        pos_point.scale.y = 0.5;
        pos_point.scale.z = 0.5;
        pos_point.color.a = 1;
        pos_point.color = Color::Red();
        pos_point.pose.position.x = x[i][0];
        pos_point.pose.position.y = x[i][1];
        pos_point.pose.position.z = x[i][2];
        ...
        p_list.markers.push_back(pos_point);
        v_list.markers.push_back(vel_vec);
        a_list.markers.push_back(acc_vec);
    }

    curr_exp_pos_pub_.publish(p_list);
    curr_exp_vel_pub_.publish(v_list);
    curr_exp_acc_pub_.publish(a_list);
}