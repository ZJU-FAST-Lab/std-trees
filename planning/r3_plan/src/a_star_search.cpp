...
AstarPathFinder::~AstarPathFinder()
{
    for (int i = 0; i < POOL_SIZE_(0); i++)
    {
        for (int j = 0; j < POOL_SIZE_(1); j++)
        {
            ...
            {
                // cout << "destory GridNodeMap_[i][j][k]" << endl;
                delete GridNodeMap_[i][j][k];
            }
            // cout << "destory GridNodeMap_["<<i<<"]["<<j<<"]" << endl;
            delete[] GridNodeMap_[i][j];
        }
        // cout << "destory GridNodeMap_["<<i<<"]" << endl;
        delete[] GridNodeMap_[i];
    }
    // cout << "destory GridNodeMap_" << endl;
    delete[] GridNodeMap_;
}
...
bool AstarPathFinder::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
...
bool AstarPathFinder::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    Vector3i start_idx(0, 0, 0), end_idx(0, 0, 0);
    pos_checker_ptr_->posToIndex(start_pt, start_idx);
    pos_checker_ptr_->posToIndex(end_pt, end_idx);
        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++)
                {
                    ...
                    if (!pos_checker_ptr_->validatePosSurround(neighborPtr->index))
                    {
                        continue;
                    }
    ...
    ros::Time time_2 = ros::Time::now();
    // ROS_WARN("A star fail. Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1).toSec(), num_iter);
    return false;
}
...
vector<Vector3d> AstarPathFinder::getPath()
{
    ...
    {
      Vector3d pos;
      pos_checker_ptr_->indexToPos(ptr->index, pos);
      path.push_back(pos);
    }
