...
namespace tgk_planner
{
                                ...
                                bool init, bool dynamic, double time_start)
  {
      ...
      if (reach_horizon || near_end)
      {
        ...
        if (near_end)
        {
          ...
          if (is_shot_succ_)
          {
            ...
            has_path_ = true;
            planning_time_ = (ros::Time::now() - t_ka_s).toSec();
            return REACH_END;
          }
          else
          {
    ...
    planning_time_ = (ros::Time::now() - t_ka_s).toSec();
    ROS_INFO_STREAM("k a jerk use: " << planning_time_);
                                          ...
                                          double time_to_goal)
  {
    ...
    if (!shot_traj.checkMaxVelRate(max_vel_))
    {
      return false;
    }
    // while (1)
    ...
    // while (1)
    // {
    //   if (shot_traj.checkMaxVelRate(vel_acc_))
    //   {
    //     break;
    //   }
    //   else if (n < 3)
